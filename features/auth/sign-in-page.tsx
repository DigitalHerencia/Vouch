...
import { loginSchema, verificationSchema } from '@/schemas/auth.schemas'
import { type LoginFormProps, type LoginFormValues } from '@/types/auth.types'
...
export function LoginForm({ className, redirectUrl, ...props }: LoginFormProps) {
    ...
    const [secondFactorMethod, setSecondFactorMethod] = useState<
    ...
    async function sendSecondFactorCode(): Promise<boolean> {
        ...
        if (supportsEmailCode) {
            ...
            if (sendResult.error) {
                form.setError('root', {
                    message:
                        sendResult.error.message || 'Could not send the email verification code.',
                })
                return false
            }
        ...
        if (supportsPhoneCode) {
            ...
            if (sendResult.error) {
                form.setError('root', {
                    message:
                        sendResult.error.message || 'Could not send the phone verification code.',
                })
                return false
            }
            ...
            if (awaitingSecondFactor) {
                ...
                if (!parsedVerification.success) {
                    form.setError('verificationCode', {
                        message:
                            parsedVerification.error.issues[0]?.message ||
            ...
            <FieldGroup className="gap-6 border border-neutral-800 bg-neutral-950 p-6 shadow-2xl shadow-black/30 sm:p-8">
                <div className="flex flex-col items-center text-center">
                    ...
                    <p className="mt-3 max-w-sm text-sm leading-6 text-neutral-400">
                        Access your dashboard, active Vouches, confirmation windows, and payment
                        setup status.
                    </p>
                </div>
