  ...
  return (
    <section className="mx-auto flex w-full max-w-3xl flex-col gap-4 px-6 py-8">
      ...
      <p className="text-muted-foreground">
        Identity: {status?.identityStatus ?? "unstarted"} · Adult: {status?.adultStatus ?? "unstarted"}
      </p>
      <div className="flex gap-3">
        {startIdentityAction ? <form action={startIdentityAction}><Button type="submit">Start verification</Button></form> : null}
        {reconcileAction ? <form action={reconcileAction}><Button type="submit" variant="outline">Refresh</Button></form> : null}
      </div>
    </section>
  )
}
