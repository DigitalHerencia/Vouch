//components/landing/public-page.tsx
...
const shellClassName =
  "mx-auto min-h-screen w-full max-w-7xl border-x border-neutral-900 bg-black/20 text-white"
      ...
      <section className="grid gap-10 lg:grid-cols-[1.08fr_0.92fr] lg:items-start">
        <div className="pt-4">
          ...
          <div className="mt-7 flex flex-col gap-3 sm:flex-row">
            <Button
              className="h-13 rounded-none bg-blue-700 px-7"
            ...
            <Button
              variant="outline"
              className="h-13 rounded-none px-7"
...
export function HowItWorksPage() {
  return (
    <PublicShell>
      <PageHeader
        eyebrow="Deterministic flow"
        title="How Vouch works"
        body="Vouch is intentionally narrow: create, accept, both confirm, then release. Otherwise refund, void, or non-capture."
      />
      <ProcessPanel />
      <RulePanel />
      <FinalCta />
  ...
  return (
    <PublicShell>
      <PageHeader
        ...
        body="Fees are shown before a Vouch is committed. Vouch coordinates payment outcomes through provider infrastructure and does not claim escrow status."
      />
...
export function FaqPage() {
  const items = [
    [
      ...
      "No. Vouch does not provide listings, discovery, public profiles, categories, reviews, ratings, or booking.",
    ],
    [
      "Does Vouch hold funds?",
      "No. Payment state is provider-backed. Vouch stores references and readiness state, not raw payment details.",
    ],
    [
      "When do funds release?",
      "Only when both parties confirm presence within the confirmation window and the provider release/capture succeeds.",
    ],
    ["What if one person confirms?", "One-sided confirmation never releases funds."],
    [
      "Does Vouch resolve disputes?",
      "No. Vouch does not mediate, arbitrate, award funds, or decide who was right.",
    ],
    [
      "What happens after expiration?",
      "If both confirmations are not complete, Vouch triggers refund, void, or non-capture according to the provider flow.",
    ],
  ]
  ...
  return (
    <PublicShell>
      <PageHeader
        ...
        body="Vouch is the commitment layer, not a marketplace, scheduler, escrow provider, or judge."
      />
    ...
    <PublicShell>
      ...
      <div className="mt-8 grid gap-4">
        {sections.map((section) => (
          <Card
            key={section.heading}
            className="rounded-none border-2 border-neutral-800 bg-neutral-950/70"
          >
            <CardHeader>
  ...
  return (
    <section className="mt-12 grid border-2 border-neutral-800 md:grid-cols-3">
      {metrics.map(([value, label, body]) => (
        <div
          key={label}
          className="border-b border-neutral-800 p-5 last:border-b-0 md:border-r md:border-b-0 md:last:border-r-0"
        >
          ...
          <Card key={item} className="rounded-none border-2 border-neutral-800 bg-neutral-950/70">
            <CardContent className="py-5">
              ...
              <p className="mt-2 text-sm text-neutral-400">
                No discovery, no booking, no provider directory.
              </p>
            </CardContent>
          </Card>
        ))}
    ...
    <section className="mt-8 grid gap-5 border-2 border-neutral-800 bg-black/60 p-6 lg:grid-cols-[auto_1fr_auto] lg:items-center">
      ...
      <div>
        ...
        <p className="mt-2 text-neutral-400">
          Vouch coordinates deterministic payment outcomes. It does not endorse users, judge
          conduct, or guarantee meeting outcomes.
        </p>
      </div>
    ...
    <Card className="mt-8 rounded-none border-2 border-blue-900 bg-blue-950/20">
      <CardContent className="grid gap-4 py-5 md:grid-cols-3">
        <div>
          ...
          <p className="mt-3 font-bold text-white">Both confirm</p>
          <p className="text-sm text-neutral-400">Funds release through provider flow.</p>
        </div>
        <div>
          <XCircle className="text-amber-500" />
          <p className="mt-3 font-bold text-white">Confirmation incomplete</p>
          <p className="text-sm text-neutral-400">Refund, void, or non-capture.</p>
        </div>
        <div>
          <LockKeyhole className="text-blue-500" />
          <p className="mt-3 font-bold text-white">No arbitration</p>
          <p className="text-sm text-neutral-400">Vouch does not decide who was right.</p>
        </div>
      </CardContent>
    </Card>
  )
    ...
    <Card className="mt-5 rounded-none border-2 border-neutral-800 bg-neutral-950/70">
      <CardContent className="flex gap-4 py-5">
        ...
        <p className="text-sm text-neutral-300">
          Payment details are handled by provider-secure flows. Vouch exposes participant-safe
          summaries only.
        </p>
      </CardContent>
    </Card>
    ...
    <>
      ...
      <section className="grid gap-4 sm:grid-cols-[1fr_auto] sm:items-center">
        <div>
          ...
          <p className="mt-2 text-neutral-400">
            Create a Vouch for an appointment or in-person agreement you already arranged.
          </p>
        </div>
        <Button
          className="h-12 rounded-none bg-blue-700 px-6"
          render={<Link href="/sign-up?return_to=/vouches/new" />}
        >
          Get started
          <ArrowRight />
