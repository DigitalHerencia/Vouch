...
export default async function DashboardRoute({ searchParams }: PageProps) {
  ...
  return <DashboardPage setupComplete={state.variant !== "empty"} sections={[
    { title: "Action required", description: "Vouches that need your attention.", vouches: (sections?.actionRequired ?? []).map(card) },
    { title: "Active", description: "Accepted Vouches awaiting confirmation or resolution.", vouches: (sections?.active ?? []).map(card) },
    { title: "Pending", description: "Created Vouches awaiting acceptance.", vouches: (sections?.pending ?? []).map(card) },
    { title: "Completed", description: "Final Vouches where both parties confirmed.", vouches: (sections?.completed ?? []).map(card) },
  ]} />
}
