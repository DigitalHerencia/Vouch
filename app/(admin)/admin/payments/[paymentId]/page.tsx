...
export default async function Page({ params }: PageProps) {
  ...
  return <AdminTablePage title="Admin Payment Detail" description="Provider-backed payment state." columns={[
    { key: "id", label: "ID" },
    { key: "provider", label: "Provider" },
    { key: "status", label: "Status" },
    { key: "lastErrorCode", label: "Error" },
  ]} rows={payment ? [{ id: String(payment.id), provider: String(payment.provider), status: String(payment.status), lastErrorCode: String(payment.lastErrorCode ?? "") }] : []} />
}
