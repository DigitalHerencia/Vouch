...
function extractClerkUserEmail(data: ClerkWebhookUserData): string | undefined {
  const primary = data.email_addresses?.find((email) => email.id === data.primary_email_address_id)
  return primary?.email_address ?? data.email_addresses?.[0]?.email_address
}

function extractClerkUserPhone(data: ClerkWebhookUserData): string | undefined {
...
function extractClerkDisplayName(data: ClerkWebhookUserData): string | undefined {
