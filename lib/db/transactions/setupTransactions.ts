import "server-only"

type Tx = unknown

// Auto-generated transaction stubs. Replace Tx with Prisma.TransactionClient after generated client is available.

export async function acceptTermsTx(_tx: Tx, _input?: unknown): Promise<never> {
  throw new Error($fn not implemented in lib/db/transactions/setupTransactions.ts)
}

export async function ensureTermsAcceptanceTx(_tx: Tx, _input?: unknown): Promise<never> {
  throw new Error($fn not implemented in lib/db/transactions/setupTransactions.ts)
}

export async function updateSetupGateSnapshotTx(_tx: Tx, _input?: unknown): Promise<never> {
  throw new Error($fn not implemented in lib/db/transactions/setupTransactions.ts)
}

export async function markSetupBlockedTx(_tx: Tx, _input?: unknown): Promise<never> {
  throw new Error($fn not implemented in lib/db/transactions/setupTransactions.ts)
}

export async function markSetupReturnedFromInviteTx(_tx: Tx, _input?: unknown): Promise<never> {
  throw new Error($fn not implemented in lib/db/transactions/setupTransactions.ts)
}

export async function markSetupReturnedFromCreateTx(_tx: Tx, _input?: unknown): Promise<never> {
  throw new Error($fn not implemented in lib/db/transactions/setupTransactions.ts)
}
